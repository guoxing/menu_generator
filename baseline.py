...
train_file = 'data/train_small'
dev_file = 'data/dev_small'
...
wsize = 3
X_train, y_train, words_train = du.generateData(train_file, word_to_num, tag_to_num, wsize)
...
X_dev, y_dev, words_dev = du.generateData(dev_file, word_to_num, tag_to_num, wsize)
DEV_SIZE = len(X_dev)
Xd_s, yd_s, wd_s = dataSubset(DEV_SIZE, X_dev, y_dev, words_dev)
fvdev = extractFeatures(Xd_s, wd_s, wsize, word_to_num)
yp = lr.predict(fvdev)
print nerbase.full_report(yd_s, yp, tagnames)
