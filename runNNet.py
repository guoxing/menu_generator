...
def run(args=None):
    ...
    if evaluate_accuracy_while_training:
        ...
        # TODO: Plot train/dev_accuracies here

        dataout = "models/%s_mvec_%s_wvec_dev_%s_epoch" % (opts.middleDim, opts.wvecDim,opts.epochs)
        with open(dataout, 'w') as fw:
            fw.write('%s\n' % dev_accuracies[-1])
            fw.write('%s\n' % opts.middleDim)
            fw.write('%s\n' % opts.wvecDim)
            fw.write('%s\n' % opts.epochs)
        ...
        plt.ylabel('accuracy')
        plt.legend(loc=2)
        plt.savefig("figures/rnn3_acc_%s_middle_%s.png" % (opts.epochs, opts.middleDim))
        plt.show()
...
def makeconf(conf_arr):
    ...
    plt.savefig("figures/dev_rnn3_10.png")
