...
def canonicalize_word(word, wordset=None, digits=True):
    # word = word.lower()
    if digits:
        if (wordset != None) and (word.lower() in wordset): return word
...
def load_and_filter_dataset(fname):
    docs = []
    with open(fname) as fd:
        cur = []
        for line in fd:
...
def docs_to_data(docs, word_to_num, tag_to_num):
    docs = flatten1([pad_sequence(seq, left=pad, right=pad) for seq in docs])
    words, tags = zip(*docs)
    words = [canonicalize_word(w, word_to_num) for w in words]
    tags = [t.split("|")[0] for t in tags]
...
###
# for project use: need intermediate words. 
###
def docs_to_data(docs, word_to_num, tag_to_num, wsize=3):
    pad = (wsize - 1)/2
    docs = flatten1([pad_sequence(seq, left=pad, right=pad) for seq in docs])
...
# filter out reviews with no entities labeled
def docs_filtered(docs):
    docf = []
    for seq in docs:
        if len(zip(*seq)) < 2:
            docf.append(seq)
